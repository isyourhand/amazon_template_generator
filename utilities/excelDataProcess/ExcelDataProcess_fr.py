...
from utilities.random_Word.random_Word_fr import lifestyle_random, fabric_type_random, collection_name_random, seasons_random, style_name_random, neck_style_random, closure_type_random, sleeve_type_random, lining_description_random, theme_random, fit_type_random, top_style_random, lifecycle_supply_type_random, occasion_types_random, waist_style_random, item_weight_random, bottom_style_random, leg_style_random, strap_type_random, fabric_wash_random, weave_type_random, collar_style_random, outer_material_type_random, item_length_description_random, inner_material_type_random, material_type_random, sport_type_random, pocket_description_random, front_style_random,rise_style_random
from utilities.excelDataProcess.excel_utils import load_and_validate_excel_data

from DataMap.DataMapping_fr import neck_style_map, closure_map, us_size_map_Mapping, fr_color_Map, fr_color_map_Map, column_map, us_Size_Mapping, Product_Type_Map_New, department_name_Map, site_Map, age_group_map, gender_map, pattern_map, column_headers

def copy_data_fr(target_file_path, source_file_path, prefix, 
    ...
    try:
        ...
        for i, item in enumerate(data, start=4):
            ...
            price_with_margin = calculate_Price(price_Hash["法国"], shipping_cost, profit)
            temp_link = item["代理链接"][0]
            item["代理链接"][0] = item["代理链接"][1]
            item["代理链接"][1] = temp_link
            
            # IF
            recommended_browse_nodes = "" if not nodes_array else random.choice(nodes_array)
            ...
            _color_ = item["颜色"] if item["颜色"].lower() not in fr_color_Map else fr_color_Map[item["颜色"].lower()]
            _color_map_ = item["颜色"] if item["颜色"].lower() not in fr_color_map_Map else fr_color_map_Map[item["颜色"].lower()]
            size_class = "Alpha/lettres" if not has_numbers(item['尺寸']) else "Âge"
            height_type = "Taille normale" if not has_numbers(item['尺寸']) else ""
            ...
            target_sheet.cell(row=i, column=column_indices["recommended_browse_nodes"], value=recommended_browse_nodes)
            target_sheet.cell(row=i, column=column_indices["closure_type"], value=closure_map[closure_type])
            target_sheet.cell(row=i, column=column_indices["language_value1"], value="Français")
            target_sheet.cell(row=i, column=column_indices["language_value2"], value="Anglais")
            target_sheet.cell(row=i, column=column_indices["league_name"], value="World Cup Soccer")
            target_sheet.cell(row=i, column=column_indices["other_image_url_ps06"], value=gpsr)
            ...
            target_sheet.cell(row=i, column=column_indices["care_instructions"], value="Lavage en machine ou Lavage à la main")
            target_sheet.cell(row=i, column=column_indices["country_of_origin"], value="Chine")
            target_sheet.cell(row=i, column=column_indices["sport_type1"], value=sport_type_random())
            target_sheet.cell(row=i, column=column_indices["currency"], value="EUR")
            ...
            target_sheet.cell(row=i, column=column_indices["band_size_num_unit_of_measure"], value="CM")
            ...
            target_sheet.cell(row=i, column=column_indices["water_resistance_level"], value="Résistant à l'eau")
            ...
            target_sheet.cell(row=i, column=column_indices["is_autographed"], value="Non")
            ...
            for idx in range(1, 11):
                ...
                if column_key in column_indices:  # 确保列索引存在
                    target_sheet.cell(row=i, column=column_indices[column_key], value=lifestyle_random())

            # 定义所有的 special_features 类型
            special_features = [
                  "Respirable","Légère", "Évacuation de l'humidité", "Séchage rapide","Protection solaire","Absorbante","Convertible","Durable","Extensible","Réversible"
            ]
            ...
            target_sheet.cell(row=i, column=column_indices["seasons"], value=seasons_random())
            target_sheet.cell(row=i, column=column_indices["collection_name"], value=collection_name_random())
            ...
            # Cloth detail
            target_sheet.cell(row=i, column=column_indices["neck_style"], value=neck_style_map[neck_style])
            ...
            target_sheet.cell(row=i, column=column_indices["outer_material_type"], value=outer_material_type_random())
            target_sheet.cell(row=i, column=column_indices["material_composition"], value=fabric_add_prefix(item["水洗唛材质"]))
            ...
            target_sheet.cell(row=i, column=column_indices["material_type1"], value="Coton")
            target_sheet.cell(row=i, column=column_indices["fabric_type"], value=fabric_type_random())
            ...
            target_sheet.cell(row=i, column=column_indices["material_type2"], value="Cachemire")
            ...
            target_sheet.cell(row=i, column=column_indices["condition_type"], value="Neuf")

            # Write Brand
            target_sheet.cell(row=i, column=column_indices["brand_name"], value=brand)
            target_sheet.cell(row=i, column=column_indices["manufacturer"], value=brand + " " + Product_Type_Map_New[product_Type] + " Usine")
            ...
            target_sheet.cell(row=i, column=column_indices["item_name"], value=generate_Title(key_word_array1, key_word_array2, Product_Type_Map_New[product_Type], pattern_map[random.choice(patterns)], _color_map_, size, item["水洗唛材质"], title_include_fields))
            ...
            if contains_alpha:  # Child
                ...
                target_sheet.cell(row=i, column=column_indices["map_price"], value=price_with_margin+10)
                target_sheet.cell(row=i, column=column_indices["sale_price"], value=price_with_margin-1)
                target_sheet.cell(row=i, column=column_indices["list_price_with_tax"], value=price_with_margin)
                ...
                target_sheet.cell(row=i, column=column_indices["apparel_body_type"], value="Taille normale")
                target_sheet.cell(row=i, column=column_indices["apparel_height_type"], value=height_type)
                target_sheet.cell(row=i, column=column_indices["apparel_size_system"], value="FR / ES")
                ...
                target_sheet.cell(row=i, column=column_indices["shirt_size_system"], value="FR / ES")
                target_sheet.cell(row=i, column=column_indices["shirt_body_type"], value="Taille normale")
                target_sheet.cell(row=i, column=column_indices["shirt_height_type"], value=height_type)
                target_sheet.cell(row=i, column=column_indices["bottoms_size_system"], value="FR / ES")
                target_sheet.cell(row=i, column=column_indices["bottoms_size_class"], value=size_class)
                target_sheet.cell(row=i, column=column_indices["bottoms_body_type"], value="Taille normale")
                target_sheet.cell(row=i, column=column_indices["bottoms_height_type"], value=height_type)
                target_sheet.cell(row=i, column=column_indices["skirt_size_system"], value="FR / ES")
                target_sheet.cell(row=i, column=column_indices["skirt_size_class"], value=size_class)
                target_sheet.cell(row=i, column=column_indices["skirt_body_type"], value="Taille normale")
                target_sheet.cell(row=i, column=column_indices["skirt_height_type"], value=height_type)
                target_sheet.cell(row=i, column=column_indices["shapewear_size_system"], value="FR / ES")
                ...
                target_sheet.cell(row=i, column=column_indices["shapewear_body_type"], value="Taille normale")
                ...
                if color:
                    ...
                    target_sheet.cell(row=i, column=column_indices["color_map"], value=_color_map_)
                    target_sheet.cell(row=i, column=column_indices["color_name"], value=color_prefix + _color_)
                ...
                part_number_value = customize_Generation(key_word_array1, key_word_array2, 33) + random_number
